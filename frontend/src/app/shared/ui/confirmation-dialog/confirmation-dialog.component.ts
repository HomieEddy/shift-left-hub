...
@Component({
  ...
  template: `
    <div class="bg-white rounded-xl shadow-xl p-6 w-[420px]">
      <h2 class="text-lg font-semibold text-slate-800 mb-2">{{ data.titleKey }}</h2>
      <p class="text-slate-600 text-sm mb-6">{{ data.messageKey }}</p>
      ...
      <div class="flex justify-end gap-3">
        ...
        >
          ...
          {{ loading() ? ('confirm.state.saving') : data.confirmLabelKey }}
        </button>
      </div>
    </div>
  `,
    ...
    if (this.data.onConfirm) {
      ...
      const result$ = from(this.data.onConfirm()).pipe(
        ...
        catchError((err) => {
          ...
          this.errorMessage.set('confirm.error.generic');
          return of(undefined);
        })
      );
