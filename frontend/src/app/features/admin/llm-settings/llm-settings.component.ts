...
import { WorkspaceLlmConfigService, WorkspaceLlmConfigResponse } from './workspace-llm-config.service';
...
export class LlmSettingsComponent implements OnInit {
  ...
  providers = computed(() => [
    ...
    { value: 'OPENAI_COMPATIBLE', label: this.translationService.translate('admin.settings.llm.provider.openai') },
  ]);
  ...
  onWorkspaceChange(): void {
    ...
    this.workspaceLlmConfigService.getConfig(wsId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (config) => {
  ...
  save(): void {
    ...
    const wsId = this.selectedWorkspaceId();
    if (wsId) {
      firstValueFrom(this.workspaceLlmConfigService.saveConfig(wsId, {
  ...
  testConnection(): void {
    ...
    firstValueFrom(obs).then(result => {
