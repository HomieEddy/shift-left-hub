...
export class WorkspaceMembersComponent implements OnInit {
  ...
  changeRole(member: WorkspaceMemberDto, newRole: string) {
    const roleLabel = this.translationService.translate('workspace.members.role.' + newRole.toLowerCase());
    this.confirmationDialog.confirm({
      title: this.translationService.translate('workspace.members.change-role-title'),
      message: this.translationService.translate('workspace.members.change-role-message', { VAR_NAME: member.displayName, VAR_ROLE: roleLabel }),
      confirmLabel: this.translationService.translate('workspace.members.change-role-confirm'),
    }).subscribe((confirmed) => {
      if (!confirmed) { this.loadData(); return; }
  ...
  confirmRemoveMember(member: WorkspaceMemberDto) {
    this.confirmationDialog.confirm({
      ...
      message: this.translationService.translate('workspace.members.remove-message', { VAR_NAME: member.displayName }),
  ...
  revokeInvitation(invitation: InvitationDto) {
    this.confirmationDialog.confirm({
      ...
      message: this.translationService.translate('workspace.members.revoke-message', { VAR_NAME: invitation.invitedUserEmail }),
