...
| O-2 | `KcsDraftingService` 399 lines, 4 responsibilities (prompt, LLM, parse, dedup) | `KcsDraftingService.java:1-399` | M | MED | ✓ fixed in `refactor/tier10-o2-kcs-drafting-service-split` (PR #123) — split into 4 single-responsibility collaborators: `KcsPromptBuilder` (LLM prompt format), `KcsResponseParser` (LLM text parsing with fallbacks), `KcsDuplicateDetector` (FTS + semantic dedup), `KcsSimilaritySearch` (keyword search for response warnings). Orchestrator `KcsDraftingService` 399 → 152 lines (−62%). 396/396 unit tests pass; 21 new tests added across the 4 new classes. |
| O-3 | `MasterSeeder` 330 lines, 5 distinct seeding steps | `MasterSeeder.java` | M | MED | ✓ fixed in `refactor/tier10-o3-master-seeder-split` (PR #124) - split into 5 single-responsibility seeders (UserSeeder, WorkspaceSeeder, WorkspaceAssignmentSeeder, AiConfigSeeder, OldArticleCleanupSeeder) + slim 90-line orchestrator. 413/413 unit tests pass (+17 from new tests). |
